...
if (!class_exists('FPD_User_Products')) {
	...
	class FPD_User_Products
	{
			...
			<script type="text/javascript">
				jQuery(document).ready(function() {
					...
					jQuery('.fpd-saved-products-grid').on('click', '.fpd-sc-remove-saved-product', function(evt) {
						...
						jQuery.post("<?php echo admin_url('admin-ajax.php'); ?>", data, function(response) {}, 'json');
		...
		public function load_user_products()
		{
			...
			//MRR - Filter Loading User saved Product Design for the Current Products
			...
			$response_data = array();
			if (!empty($saved_products)) {
				$copy_saved_products = $saved_products;
				foreach ($copy_saved_products as $key => $saved_product) {

					$saved_post_id = intval($saved_product['post_id']);

					if ($saved_post_id == intval($_POST['post_id'])) {
						$response_data[array_search($saved_product, $copy_saved_products)] = $saved_product;
					};
				};
			};
