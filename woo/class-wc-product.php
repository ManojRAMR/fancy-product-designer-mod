...
if (!class_exists('FPD_WC_Product')) {
	...
	class FPD_WC_Product
	{
		...
		// MRR - Before FPD designer load raq products to edit
		public function before_product_designer_from_raq($post)
		{
			...
			if (isset($_POST['fpd_product'])) {
				...
				//$raq_item_values = array_values($raq_item_from_request);

				$raq_item_arr = array(
					'quantity' => $raq_item_values['quantity'],
					'fpd_product_thumbnail' => urldecode($raq_item_values['fpd_product_thumbnail']),
					'fpd_product' => urldecode($raq_item_values['fpd_product'])
				);
				//print_r($raq_item_arr['fpd_product_thumbnail']);
				//print_r(urldecode($raq_item_value['0']['fpd_product_thumbnail']));
